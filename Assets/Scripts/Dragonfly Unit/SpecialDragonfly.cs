...
public class SpecialDragonfly : SpecialBase
{
    void Awake() {
    ...
    public override bool SpecialAttackTargetedUnit(GameObject enemyUnit) {
        ...
        foreach (Vector2Int special in _availableSpecials) {
            if (special == enemyCoords) {
                ...
                bool unitDodged = Random.Range(0, 101) <= enemyStats.Clarity;
                
                if (unitDodged) {
                    Debug.Log($"{enemyUnit.name} dodged the DRAGONGLY special.");
                    return true;
                }

                UnitStatus enemyStatus = enemyUnit.GetComponent<UnitStatus>();
                enemyStatus.ApplyStatus(StatusType.Dazed, 4, 0);
                Debug.Log($"{enemyUnit.transform.name} has been dazed by {transform.name}.");
