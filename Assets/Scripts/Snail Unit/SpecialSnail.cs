...
public class SpecialSnail : SpecialBase
{
    void Awake() {
        _stats = GetComponent<Stats>();
        _atk = _stats.Atk;
        _availableSpecials = new List<Vector2Int>();
        _specialPattern = new Vector2Int[22];
        ...
        _specialPattern[12] = new Vector2Int(-40, 10);
        _specialPattern[13] = new Vector2Int(-40, -10);
        _specialPattern[14] = new Vector2Int(40, 10);
        _specialPattern[15] = new Vector2Int(40, -10);

        _specialPattern[16] = new Vector2Int(-10, 40);
        _specialPattern[17] = new Vector2Int(10, 40);
        _specialPattern[18] = new Vector2Int(0, 50);

        _specialPattern[19] = new Vector2Int(-10, -40);
        _specialPattern[20] = new Vector2Int(10, -40);
        _specialPattern[21] = new Vector2Int(0, -50);
    }
    ...
    public override bool SpecialAttackTargetedUnit(GameObject enemyUnit) {
        ...
        foreach (Vector2Int special in _availableSpecials) {
            if (special == enemyCoords) {
                ...
                if (unitDodged) {
                    Debug.Log($"{enemyUnit.name} dodged the SNAIL special.");
                    return true;
                }

                int totalDamage = _atk - enemyStats.Def > 0 ? _atk - enemyStats.Def : 0;
                enemyStats.Hp -= totalDamage;
                Debug.Log($"{transform.name} attacked {enemyUnit.transform.name} using SNAIL special for {totalDamage} damage.");
